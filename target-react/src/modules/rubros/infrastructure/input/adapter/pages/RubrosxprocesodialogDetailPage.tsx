...
import { RubrosxprocesodialogDetail } from '../components/RubrosxprocesodialogDetail';
import { Loading } from '@shared/infrastructure/input/adapter/components/Loading';
import { ErrorBanner } from '@shared/infrastructure/input/adapter/components/ErrorBanner';

export const RubrosxprocesodialogDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { selectedItem, loading, error, fetchById, remove, clearError } = useRubrosxprocesodialog();
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '2rem' }}>
      <RubrosxprocesodialogDetail
        item={selectedItem}
        onEdit={(item) => navigate(`/rubrosxprocesodialog/editar/${item.id}`)}
