...
export const CargarubrosidoDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { selectedItem, loading, error, fetchById, remove, clearError } = useCargarubrosido();
  ...
  if (error) {
    return <ErrorBanner message={error} onRetry={() => { clearError(); if (id) fetchById(id); }} />;
  }
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '2rem' }}>
      <CargarubrosidoDetail
        item={selectedItem}
        onEdit={(item) => navigate(`/cargarubrosido/editar/${item.id}`)}
