...
import { EjecucionbotDetail } from '../components/EjecucionbotDetail';

export const EjecucionbotDetailPage: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { selectedItem, loading, error, fetchById, remove, clearError } = useEjecucionbot();
  ...
  const domainItem = useMemo(() => (selectedItem ? EjecucionbotViewMapper.toDomain(selectedItem) : null), [selectedItem]);
  ...
  return (
    <main className="container" data-testid="ejecucionbot-detail-page">
      {error && <ErrorBanner message={error} onRetry={() => { clearError(); if (id) { fetchById(id); } }} />}
      <EjecucionbotDetail
        item={domainItem}
        onEdit={(item) => navigate(ROUTES.PATHS['ejecucionbot'] + '/edit/' + item.id)}
        ...
        onBack={() => navigate(ROUTES.PATHS['ejecucionbot'])}
      />
    </main>
  );
};
