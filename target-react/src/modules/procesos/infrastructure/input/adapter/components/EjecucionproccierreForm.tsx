...
interface EjecucionproccierreFormProps {
  ...
  onSubmit: (data: CreateEjecucionproccierre | UpdateEjecucionproccierre) => Promise<void> | void;
  onCancel?: () => void;
  loading?: boolean;
}

function toFormState(model?: Ejecucionproccierre): Record<string, string> {
  if (!model) {
    return {};
  }
...
export const EjecucionproccierreForm: React.FC<EjecucionproccierreFormProps> = ({ initialData, onSubmit, onCancel, loading = false }) => {
  ...
  const handleSubmit = async (formValues: Record<string, string>) => {
    ...
    if (keys.length === 0) {
      try {
        ...
        await onSubmit(parsed as CreateEjecucionproccierre | UpdateEjecucionproccierre);
      } catch (_error) {
    ...
    await onSubmit(formValues as unknown as CreateEjecucionproccierre | UpdateEjecucionproccierre);
  };

  return (
    <Form methods={methods} onSubmit={handleSubmit} className="card" data-testid="ejecucionproccierre-form">
      <h3 style={{ marginTop: 0 }}>{isEditMode ? 'Editar' : 'Crear'} Ejecucionproccierre</h3>
      {keys.length > 0 ? (
        keys.map((key) => (
        ...
        <label className="form-field">
          ...
          <textarea
            ...
            required
            disabled={loading}
            data-testid="ejecucionproccierre-json"
          />
        </label>
      )}
      {error && <p className="field-error">{error}</p>}
      <div className="form-actions">
        <button type="submit" className="btn btn-primary" disabled={loading} data-testid="ejecucionproccierre-submit">{loading ? 'Guardando...' : 'Guardar'}</button>
        {onCancel && <button type="button" className="btn btn-secondary" onClick={onCancel} disabled={loading} data-testid="ejecucionproccierre-cancel">Cancelar</button>}
      </div>
    </Form>
  );
};
