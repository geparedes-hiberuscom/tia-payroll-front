...
interface RubrospreliquidadossubrecursoFormProps {
  ...
  onSubmit: (data: CreateRubrospreliquidadossubrecurso | UpdateRubrospreliquidadossubrecurso) => Promise<void> | void;
  onCancel?: () => void;
  loading?: boolean;
}

function toFormState(model?: Rubrospreliquidadossubrecurso): Record<string, string> {
  if (!model) {
    return {};
  }
...
export const RubrospreliquidadossubrecursoForm: React.FC<RubrospreliquidadossubrecursoFormProps> = ({ initialData, onSubmit, onCancel, loading = false }) => {
  ...
  const handleSubmit = async (formValues: Record<string, string>) => {
    ...
    if (keys.length === 0) {
      try {
        ...
        await onSubmit(parsed as CreateRubrospreliquidadossubrecurso | UpdateRubrospreliquidadossubrecurso);
      } catch (_error) {
    ...
    await onSubmit(formValues as unknown as CreateRubrospreliquidadossubrecurso | UpdateRubrospreliquidadossubrecurso);
  };

  return (
    <Form methods={methods} onSubmit={handleSubmit} className="card" data-testid="rubrospreliquidadossubrecurso-form">
      <h3 style={{ marginTop: 0 }}>{isEditMode ? 'Editar' : 'Crear'} Rubrospreliquidadossubrecurso</h3>
      {keys.length > 0 ? (
        keys.map((key) => (
        ...
        <label className="form-field">
          ...
          <textarea
            ...
            required
            disabled={loading}
            data-testid="rubrospreliquidadossubrecurso-json"
          />
        </label>
      )}
      {error && <p className="field-error">{error}</p>}
      <div className="form-actions">
        <button type="submit" className="btn btn-primary" disabled={loading} data-testid="rubrospreliquidadossubrecurso-submit">{loading ? 'Guardando...' : 'Guardar'}</button>
        {onCancel && <button type="button" className="btn btn-secondary" onClick={onCancel} disabled={loading} data-testid="rubrospreliquidadossubrecurso-cancel">Cancelar</button>}
      </div>
    </Form>
  );
};
