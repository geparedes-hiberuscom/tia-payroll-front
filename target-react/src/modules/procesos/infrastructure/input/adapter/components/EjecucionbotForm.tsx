...
interface EjecucionbotFormProps {
  initialData?: Ejecucionbot;
  onSubmit: (data: CreateEjecucionbot | UpdateEjecucionbot) => Promise<void> | void;
  onCancel?: () => void;
  loading?: boolean;
}

function toFormState(model?: Ejecucionbot): Record<string, string> {
  if (!model) {
    return {};
  }
...
export const EjecucionbotForm: React.FC<EjecucionbotFormProps> = ({ initialData, onSubmit, onCancel, loading = false }) => {
  ...
  const handleSubmit = async (formValues: Record<string, string>) => {
    ...
    if (keys.length === 0) {
      try {
        ...
        await onSubmit(parsed as CreateEjecucionbot | UpdateEjecucionbot);
      } catch (_error) {
    ...
    await onSubmit(formValues as unknown as CreateEjecucionbot | UpdateEjecucionbot);
  };

  return (
    <Form methods={methods} onSubmit={handleSubmit} className="card" data-testid="ejecucionbot-form">
      <h3 style={{ marginTop: 0 }}>{isEditMode ? 'Editar' : 'Crear'} Ejecucionbot</h3>
      {keys.length > 0 ? (
        keys.map((key) => (
        ...
        <label className="form-field">
          ...
          <textarea
            ...
            required
            disabled={loading}
            data-testid="ejecucionbot-json"
          />
        </label>
      )}
      {error && <p className="field-error">{error}</p>}
      <div className="form-actions">
        <button type="submit" className="btn btn-primary" disabled={loading} data-testid="ejecucionbot-submit">{loading ? 'Guardando...' : 'Guardar'}</button>
        {onCancel && <button type="button" className="btn btn-secondary" onClick={onCancel} disabled={loading} data-testid="ejecucionbot-cancel">Cancelar</button>}
      </div>
    </Form>
  );
};
