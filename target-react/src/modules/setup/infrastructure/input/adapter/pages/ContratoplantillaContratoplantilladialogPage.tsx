...
import { ContratoplantillaContratoplantilladialogList } from '../components/ContratoplantillaContratoplantilladialogList';
...
export const ContratoplantillaContratoplantilladialogPage: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '2rem' }}>
      ...
      {!showForm && (loading && items.length === 0 ? <Loading message="Cargando..." /> : <ContratoplantillaContratoplantilladialogList items={items} loading={loading} onEdit={(item) => { setEditingItem(item); setShowForm(true); }} onDelete={(id) => { if (window.confirm('¿Eliminar?')) remove(id.toString()); }} />)}
    </div>
  );
};
