...
export const PlantillacontablePlantillacontabledialogPage: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '2rem' }}>
      ...
      {!showForm && (loading && items.length === 0 ? <Loading message="Cargando..." /> : <PlantillacontablePlantillacontabledialogList items={items} loading={loading} onEdit={(item) => { setEditingItem(item); setShowForm(true); }} onDelete={(id) => { if (globalThis.confirm('¿Eliminar?')) remove(id); }} />)}
    </div>
  );
};
