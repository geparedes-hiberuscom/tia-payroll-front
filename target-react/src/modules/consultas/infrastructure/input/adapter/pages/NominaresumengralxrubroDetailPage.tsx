...
import { NominaresumengralxrubroDetail } from '../components/NominaresumengralxrubroDetail';
import { Loading } from '@shared/infrastructure/input/adapter/components/Loading';
import { ErrorBanner } from '@shared/infrastructure/input/adapter/components/ErrorBanner';

/**
 * Página de detalle: nominaresumenGralxRubro.zul
 * Muestra el detalle completo de un registro.
 *
 * TODO: Copilot — Completar con la navegación y acciones específicas.
 */
export const NominaresumengralxrubroDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { selectedItem, loading, error, fetchById, remove, clearError } = useNominaresumengralxrubro();
  ...
  const handleEdit = () => {
    navigate(`/nominaresumengralxrubro/${id}/edit`);
  };
